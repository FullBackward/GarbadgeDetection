...
input_path = "../data/COCO/"
output_path = "../data/YOLO/"

f = open(input_path+"/annotations.json")
data = json.load(f)
f.close()
...
def load_images_from_folder(folder, count):
    dirc = input_path + folder 
    for filename in os.listdir(dirc):
        source = os.path.join(dirc,filename)
...
for i in range(15):
    count = load_images_from_folder('batch_' + str(i+1), count)
